...
# Инициализация AI модели (замена для обработки запросов Qwen)
def analyze_input_with_qwen(user_input, TAGS_DICT):
    """
    Анализирует текст запроса для извлечения параметров поиска (меток, цены, лимита).
    """
    ...
    try:
        ...
        # Добавляем пользовательский запрос в конец инструкции
        ...
        #else:
            #print(f"Неправильный формат ответа: {result}")
            #return None

    except Exception as e:
        print(f"Ошибка обработки текста через Qwen: {e}")
        return None
...
def fetch_games_by_tags_and_price(tags, price_range=None, limit=50):
    ...
    for tag in tags:
        ...
        if tag_upper in TAGS_DICT:
            ...
            fetched_id = fetch_tag_id(tag_upper)
            if fetched_id:
...
def search_games(user_input):
    ...
    #    for game in games:
    #        result += f"Название: {game['Название']}, Цена: {game['Цена']}, Метки: {', '.join(game['Метки'])}, Ссылка: <a href=\"{game['Ссылка']}\" target=\"_blank\">{game['Ссылка']}</a>\n"

    #    return result
    #else:
    #    return "Подходящих игр не найдено."
